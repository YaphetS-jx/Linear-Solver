...
void Initialize(DS_AAR* pAAR) {
    ...
    pAAR->phi_guess = (double***) calloc((pAAR->np_z+2*pAAR->FDn) , sizeof(double**)); // need to de-allocate later  
    pAAR->phi = (double***) calloc((pAAR->np_z+2*pAAR->FDn) , sizeof(double**)); // need to de-allocate later
    assert(pAAR->phi_guess != NULL && pAAR->phi != NULL);

    for (k=0;k<pAAR->np_z+2*pAAR->FDn;k++) {
        pAAR->phi_guess[k] = (double**) calloc((pAAR->np_y+2*pAAR->FDn) , sizeof(double*)); // need to de-allocate later
        pAAR->phi[k] = (double**) calloc((pAAR->np_y+2*pAAR->FDn) , sizeof(double*)); // need to de-allocate later
        assert(pAAR->phi_guess[k] != NULL && pAAR->phi[k] != NULL);

        for (j=0;j<pAAR->np_y+2*pAAR->FDn;j++) {
            pAAR->phi_guess[k][j] = (double*) calloc((pAAR->np_x+2*pAAR->FDn) , sizeof(double)); // need to de-allocate later
            pAAR->phi[k][j] = (double*) calloc((pAAR->np_x+2*pAAR->FDn) , sizeof(double)); // need to de-allocate later
            assert(pAAR->phi_guess[k][j] != NULL && pAAR->phi[k][j] != NULL);
        }
    }
...
void PoissonResidual(DS_AAR* pAAR,double ***phi_old,double ***phi_res,int iter,MPI_Comm comm_dist_graph_cart, int **eout_s,int **eout_e, int **ein_s,int **ein_e,
                    int ereg_s[3][26],int ereg_e[3][26],int **stencil_sign,int **edge_ind,int *displs_send,int *displs_recv,int *ncounts_send,int *ncounts_recv) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD,&rank);
    // STEPS TO FOLLOW:
...
void Deallocate_memory(DS_AAR* pAAR)  {
    ...
    for (k=0;k<pAAR->np_z+2*pAAR->FDn;k++) {
        for (j=0;j<pAAR->np_y+2*pAAR->FDn;j++) {
            free(pAAR->phi_guess[k][j]);
            free(pAAR->phi[k][j]);
        }
        free(pAAR->phi_guess[k]);
        free(pAAR->phi[k]);
    }
    free(pAAR->phi_guess);
    free(pAAR->phi);
